...
def run_eval(limit=20):
    import json
    dev_file = os.path.expanduser("~/AgentFlow/data/spider_data/dev.json")
    with open(dev_file) as f:
        dataset = json.load(f)
    dataset = dataset[:limit]
    solver = construct_solver(llm_engine_name="together-Qwen/Qwen2.5-7B-Instruct-Turbo")
    correct, total = 0, 0
    for sample in dataset:
        ...
        if set(map(tuple, pred_rows)) == set(map(tuple, gold_rows)):
            correct += 1
        total += 1
        print(f"[{total}/{limit}] Acc: {correct/total:.3f} | Q: {question[:50]}")
